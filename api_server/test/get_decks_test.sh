...
curl -X POST "http://0.0.0.0/groups?id=testGroup2&owner_id=test" -H  "accept: application/json"
curl -X POST "http://localhost/group/testGroup2/invite?user_id=test1" -H  "accept: application/json"
curl -X POST "http://localhost/user/test1/accept_group?group_id=testGroup2" -H  "accept: application/json"
curl -X POST "http://localhost/group/testGroup2/deck?deck_id=testDeck&description=This%20is%20a%20test%20deck&owner_id=test" -H  "accept: application/json"
curl -X GET "http://localhost/user/test/decks" -H  "accept: application/json"